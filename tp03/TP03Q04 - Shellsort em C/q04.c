...
int shellSort(Serie *list, int length)
{
    int comparisons = 0;
    for (int gap = length / 2; gap > 0; gap /= 2)
    {
        ...
        {
            Serie newElement = list[i];
            int j = i;
            while (j >= gap && (strcmp(list[j - gap].idioma, newElement.idioma) > 0) ||
                   (strcmp(list[j - gap].idioma, newElement.idioma) == 0 &&
                    strcmp(list[j - gap].nome, newElement.nome) > 0))
            {
                list[j] = list[j - gap];
                j -= gap;
                comparisons++;
...
int main()
{
    ...
    int comparisons = shellSort(serieList, i);
